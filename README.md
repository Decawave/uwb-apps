...
9. The above examples are the preverbal 'Hello World' of RTLS. 

Next, have a look at the twr_node/tag_tdma example that implements a synchronous slotted network, and the twr_node/tag_nranges_tdma examples that implement an efficiency ranging scheme for TDOA applications. 

The twr_node/tag_ota_ble examples work in conjunction with the Mynewt Nimble stack and leverage BLE for diagnosis: noteworthy is the 'Adafruit Mynewt monitor app' that provides a windows on the statistics feature of Mynewt over BLE.

The Listener example is a useful debug tool that promiscuously snoops the air and capture all packets.
